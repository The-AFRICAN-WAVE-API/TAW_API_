...
export async function fetchPopularSocialPosts() {
  ...
  const query = popularHashtags.map((tag) => `#${tag}`).join(" OR ");
  ...
  const url = `https://api.twitter.com/2/tweets/search/recent?query=${encodeURIComponent(
    query
  )}&tweet.fields=public_metrics,entities,attachments&expansions=attachments.media_keys&media.fields=url`;
  const options = {
    headers: {
    ...
    if (data && Array.isArray(data.data)) {
      const tweets = data.data
        ...
        .filter((tweet) => {
          ...
          const popularByRetweets =
            metrics.retweet_count && metrics.retweet_count > 5;
          const popularByLikes = metrics.like_count && metrics.like_count > 50;
          const hasEntities =
            tweet.entities && Object.keys(tweet.entities).length > 0;
...
export async function processAndStoreSocialPosts() {
  ...
  if (operationCount > 0) {
    await batch.commit();
    console.log(
      "Final batch commit executed for social posts, remaining count:",
      operationCount
    );
  }
  return {
    message: "Social media posts ingested successfully",
    count: posts.length,
  };
}
